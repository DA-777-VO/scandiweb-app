...
-- ── Products ────────────────────────────────────────────────────────────────
-- All data matches data.json exactly
...
('huarache-x-stussy-le', 'Nike Air Huarache Le', 1,
 ...
 'clothes', 'Nike x Stussy'),

-- FIX: gallery had only 5 images, data.json has 7
...
-- FIX: description was '<p>Xbox Series S 512GB</p>', data.json has full German HTML
...
-- FIX: description was truncated, data.json has full multi-section HTML
...
-- FIX: description was missing the <p> paragraph, data.json has both <h1> and <p>
...
-- iMac: Touch ID in keyboard (text)
...
-- AirPods Pro and AirTag have no attributes (empty array in data.json)
